...
down_revision: str | Sequence[str] | None = "d74f2f5a7a56"
...
def downgrade() -> None:
    ...
    op.add_column(
        "m_user", sa.Column("bio", sa.VARCHAR(length=1000), autoincrement=False, nullable=True)
    )
    op.add_column(
        "m_user", sa.Column("full_name", sa.VARCHAR(length=100), autoincrement=False, nullable=True)
    )
